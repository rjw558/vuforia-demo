...
package com.hermesinnovationlab.hermesplay.videoplayback;
...
import com.hermesinnovationlab.hermesplay.ui.VideoPlaybackActivity;
import com.hermesinnovationlab.hermesplay.vuforia.VuforiaRenderer;
import com.hermesinnovationlab.hermesplay.vuforia.VuforiaRendererControl;
import com.hermesinnovationlab.hermesplay.vuforia.VuforiaSession;
import com.hermesinnovationlab.hermesplay.vuforia.utils.VuforiaMath;
import com.hermesinnovationlab.hermesplay.vuforia.utils.VuforiaUtils;
import com.hermesinnovationlab.hermesplay.vuforia.utils.Texture;
...
import static com.hermesinnovationlab.hermesplay.ui.VideoPlaybackActivity.TARGET_NAME_STONES;

// The renderer class for the VideoPlayback sample.
public class VideoPlaybackRenderer implements GLSurfaceView.Renderer, VuforiaRendererControl {

    private static final String LOGTAG = "VideoPlaybackRenderer";

    VuforiaSession vuforiaAppSession;
    VuforiaRenderer mSampleAppRenderer;

    private boolean watermarkVisible = true;

    // Scale for video playback.
    // Bigger values = smaller video.
    float videoPlaybackScale = 0.8f;
    ...
    public int videoPlaybackTextureID[] = new int[VideoPlaybackActivity.NUM_TARGETS];
    ...
    public Vec3F[] targetPositiveDimensions = new Vec3F[VideoPlaybackActivity.NUM_TARGETS];
    ...
    VideoPlaybackActivity mActivity;

    // Needed to calculate whether a screen tap is inside the target
    Matrix44F modelViewMatrix[] = new Matrix44F[VideoPlaybackActivity.NUM_TARGETS];

    private Vector<Texture> mTextures;

    boolean isTracking[] = new boolean[VideoPlaybackActivity.NUM_TARGETS];
    VideoPlayerHelper.MEDIA_STATE currentStatus[] = new VideoPlayerHelper.MEDIA_STATE[VideoPlaybackActivity.NUM_TARGETS];

    // These hold the aspect ratio of both the video and the
    // keyframe
    float videoQuadAspectRatio[] = new float[VideoPlaybackActivity.NUM_TARGETS];
    float keyframeQuadAspectRatio[] = new float[VideoPlaybackActivity.NUM_TARGETS];

    public VideoPlaybackRenderer(VideoPlaybackActivity activity,
        VuforiaSession session)
    {
        ...
        mVideoPlayerHelper = new VideoPlayerHelper[VideoPlaybackActivity.NUM_TARGETS];
        mMovieName = new String[VideoPlaybackActivity.NUM_TARGETS];
        mCanRequestType = new VideoPlayerHelper.MEDIA_TYPE[VideoPlaybackActivity.NUM_TARGETS];
        mSeekPosition = new int[VideoPlaybackActivity.NUM_TARGETS];
        mShouldPlayImmediately = new boolean[VideoPlaybackActivity.NUM_TARGETS];
        mLostTrackingSince = new long[VideoPlaybackActivity.NUM_TARGETS];
        mLoadRequested = new boolean[VideoPlaybackActivity.NUM_TARGETS];
        mTexCoordTransformationMatrix = new float[VideoPlaybackActivity.NUM_TARGETS][16];

        // Initialize the arrays to default values
        for (int i = 0; i < VideoPlaybackActivity.NUM_TARGETS; i++)
        {
    ...
    public void onDrawFrame(GL10 gl)
    {
        ...
        for (int i = 0; i < VideoPlaybackActivity.NUM_TARGETS; i++)
        {
            ...
            if (isTracking(i))
            {
                if (watermarkVisible == true) {
                    Log.d(mActivity.toString(), "mActivity isTracking");
            ...
            } else
            {
                ...
                {
                    // If it's been more than 0.5 seconds then pause the player
                    if ((SystemClock.uptimeMillis() - mLostTrackingSince[i]) > 500)
                    {
                        if (watermarkVisible == false) {
                            Log.d(mActivity.toString(), "mActivity isTracking");
                            mActivity.runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
    ...
    void initRendering()
    {
        ...
        videoPlaybackShaderID = VuforiaUtils.createProgramFromShaderSrc(
        ...
        keyframeShaderID = VuforiaUtils.createProgramFromShaderSrc(
...
//        keyframeQuadAspectRatio[VideoPlaybackActivity.STONES] = (float) mTextures
//            .get(0).mHeight / (float) mTextures.get(0).mWidth;
        keyframeQuadAspectRatio[VideoPlaybackActivity.CHIPS] = (float) mTextures.get(0).mHeight
    ...
    public void renderFrame(State state, float[] projectionMatrix)
    {
        ...
        for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++)
        {
            ...
            // calculation
            if (imageTarget.getName().compareTo(TARGET_NAME_STONES) == 0)
                currentTarget = VideoPlaybackActivity.CHIPS;
            else
                currentTarget = VideoPlaybackActivity.CHIPS;
            ...
            temp[0] = targetPositiveDimensions[currentTarget].getData()[0] / videoPlaybackScale;
            temp[1] = targetPositiveDimensions[currentTarget].getData()[1] / videoPlaybackScale;
            ...
            VuforiaUtils.checkGLError("VideoPlayback renderFrame");
        }
    ...
    public boolean isTapOnScreenInsideTarget(int target, float x, float y)
    {
        ...
        intersection = VuforiaMath.getPointToPlaneIntersection(VuforiaMath
        ...
        float[] textureCoordMatrix)
    {
        ...
        if (target == VideoPlaybackActivity.CHIPS)
        {
